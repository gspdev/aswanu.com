...
// change position of dealproducts on small screen device
jQuery(document).ready(function ($) {
    var tmpDocW = $(document).width();
    var rb = "<div class='col-xs-12 visible-xs hidden-sm newSidebar'>" + $(".main-right-box").html() + "</div>";
    addRb();
    $(".main-right-box>.sidebar .dealproducts .products-grid").find("li.item:gt(2)").css({ 'display': 'none' });
    $(window).resize(function () {
        addRb();
    });
    function addRb() {
        if (tmpDocW <= 767 && $(".newSidebar").index() < 0) {
            $(".col2-right-layout>.main>.row").find(".main-left-box").parent().prepend(rb);
        }
    }
});
...
function loadingAjaxcart(show) {
    var smallScreen = jQuery(document).width() < 768 ? true : false;
    var inboxCart = jQuery('.wrapper_inboxCart');
    if(smallScreen){
        //at small screen
        if(show){

            inboxCart.stop(true).html('<div class ="ajaxcartReponse"><p class="info"><i class="fa fa fa-circle-o-notch fa-spin" aria-hidden="true"></i>Loading</p></div>').fadeIn().animate({ "bottom": "20px" });

        }else{
            
            inboxCart.stop(true).animate({"bottom":"-60px"}).fadeOut();
        }
    }else{
        //not small screen
        if(show){

            console.log("PC: loadingAjaxcart true.");
            inboxCart.stop(true).html('<div class ="ajaxcartReponse"><p class="info"><i class="fa fa fa-circle-o-notch fa-spin" aria-hidden="true"></i>Loading</p></div>').fadeIn(function(){
                inboxCart.addClass("active");
            }).animate({"top":"115px"}).css("top","115px");
             
        }else{

            console.log("PC: loadingAjaxcart false.");
            inboxCart.stop(true).animate({"top":"-60px"}).fadeOut();
        }
    }
}
...
function showBoxInfo(product_info) {
    ...
    var smallScreen = jQuery(document).width() < 768 ? true : false;
    if (smallScreen) {
        inboxCart.stop(true).html(product_info).fadeIn().animate({ "bottom": "20px" }).delay(2000).animate({ "bottom": "-60px" }).fadeOut();
    } else if (!smallScreen) {
        if(inboxCart.hasClass("active")){
            console.log("PC: have active");
            inboxCart.html(product_info).delay(2000).animate({"top":"-60px"}).fadeOut();
        }else{
            console.log("PC: not have active");
            inboxCart.stop(true).html(product_info).fadeIn().animate({"top":"115px"}).delay(2000).animate({"top":"-60px"}).fadeOut();
        }
    }
...
function ajaxToCart(url, data, mine) {
    ...
    try {
        jQuery.ajax({
            ...
            data: data,
            beforeSend: function () {
                loadingAjaxcart(true);//show loading animate
            },
            success: function (data) {
                ...
                if (data.status == 1) {
                    ...
                    if (data.product_info) {
                        ...
                        showBoxInfo('<div class ="ajaxcartReponse"><p class ="info"><i class="fa fa-check-circle" aria-hidden="true"></i>Item is added to your cart</p></div>');
                    } else {
                        loadingAjaxcart(false);
                    }
