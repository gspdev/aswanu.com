...
class Unirgy_Dropship_Model_Source extends Unirgy_Dropship_Model_Source_Abstract
{
    ...
    const LABEL_SETTINGS_GLOBAL = 0;
    const LABEL_SETTINGS_VENDOR = 1;
    const LABEL_SETTINGS_THIRDPARTY = 2;
    ...
    public function toOptionHash($selector=false)
    {
        ...
        case 'udropship/vendor/vendor_sender_name_field':
        case 'udropship/vendor/vendor_sender_email_field':
            $options = $this->getVendorVisiblePreferences();
            array_unshift($options, array('value'=>'', 'label'=>Mage::helper('udropship')->__('* Use Vendor Email')));
            break;

        case 'udropship/vendor/vendor_email_identity':
            $options = Mage::getSingleton('adminhtml/system_config_source_email_identity')->toOptionArray();
            array_push($options, array('value'=>'*vendor_field*', 'label'=>Mage::helper('udropship')->__('* Use Vendor Field')));
            break;
        ...
        case 'use_label_settings':
            $options = array(
                self::LABEL_SETTINGS_GLOBAL=>Mage::helper('udropship')->__('Global If Enabled'),
                self::LABEL_SETTINGS_VENDOR=>Mage::helper('udropship')->__('Vendor'),
                self::LABEL_SETTINGS_THIRDPARTY=>Mage::helper('udropship')->__('Only Vendor Third-Party fields'),
            );
            break;
