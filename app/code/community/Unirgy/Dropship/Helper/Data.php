    ...
    */
    ...
    public function sendPasswordResetEmail($email)
    {
        ...
        Mage::getModel('core/email_template')->sendTransactional(
            ...
            Mage::helper('udropship')->getVendorEmailIdentity($vendor, $store),
            $email, $email, array(
    ...
    public function getVendorEmailIdentity($vendor, $store=null)
    {
        $store = Mage::app()->getStore($store);
        $identity = $store->getConfig('udropship/vendor/vendor_email_identity');
        if ($identity == '*vendor_field*') {
            $senderNameField = $store->getConfig('udropship/vendor/vendor_sender_name_field');
            $senderEmailField = $store->getConfig('udropship/vendor/vendor_sender_email_field');
            if ($vendor->getData($senderEmailField)) {
                $identity = array(
                    'name' => $vendor->getData($senderNameField),
                    'email' => $vendor->getData($senderEmailField)
                );
            } else {
                $identity = 'general';
            }
    ...
    public function getVendorSenderIdentity($vendor, $store=null)
    {
        $sender = $_sender = $this->getVendorEmailIdentity($vendor, $store);
        if (!is_array($sender)) {
            $sender['name'] = $vendor->getVendorName();
            $sender['email'] = $vendor->getEmail();
        }
        return $sender;
    }
    ...
    */
    public function sendVendorComment($shipment, $comment)
    {
        ...
        if ($subject && $template && $vendor->getId()) {
            ...
            $vendorSender = Mage::helper('udropship')->getVendorSenderIdentity($vendor, $store);

            $mail = Mage::getModel('core/email')
                ->setFromEmail($vendorSender['email'])
                ->setFromName($vendorSender['name'])
                ->setToEmail($toEmail)
                ->setToName($toName)
                ->setSubject($subject)
                ->setBody($template)
...
#echo "<xmp>"; echo $response; exit;
    ...
    */
    ...
    protected function _loadCustomData($obj, $filter=array())
    {
        ...
        if ($obj->getCustomVarsCombined()) {
            ...
            if (strpos($varsCombined, 'a:')===0) {
                ...
                $vars = Mage::helper('core')->jsonDecode($varsCombined);
            }
    ...
    public function processCustomVars($obj)
    {
        ...
        foreach (Mage::getConfig()->getNode('global/udropship/vendor/fields')->children() as $code=>$node) {
            $_key = $node->name ? (string)$node->name : $code;
            if ($node->is('customvars_exclude')) continue;
...
#print_r($result); echo "\n";
        ...
        foreach ($tracks as $track) {
            ...
            if (!$shipped) {
                ...
                if (!$track->getUdropshipStatus()) {
                    ...
                    $trackApi = Mage::helper('udropship')->getVendor($vendorId)->getTrackApi($track->getCarrierCode());
    ...
    public function getShippingMethods($filterWebsite=false)
    {
        $filterWebsite = $filterWebsite===true ? 'true' : ($filterWebsite===false ? 'false' : $filterWebsite);
        if (!isset($this->_shippingMethods[$filterWebsite])) {
            $this->_shippingMethods[$filterWebsite] = Mage::getModel('udropship/shipping')->getCollection();
            if ($filterWebsite!='false') {
                $wId = Mage::app()->getStore()->getWebsiteId();
                if (is_numeric($filterWebsite)) {
                    $wId = $filterWebsite;
                }
                $this->_shippingMethods[$filterWebsite]->addWebsiteFilter($wId);
            }
        }
        return $this->_shippingMethods[$filterWebsite];
    }

    protected $_systemShippingMethods=array();
    public function getSystemShippingMethods($filterWebsite=false)
    {
        if (!$this->_systemShippingMethods[$filterWebsite]) {
            $systemMethods = array();
            $shipping = $this->getShippingMethods($filterWebsite);
            ...
            $this->_systemShippingMethods[$filterWebsite] = $systemMethods;
        }
        return $this->_systemShippingMethods[$filterWebsite];
    }

    protected $_multiSystemShippingMethods=array();
    public function getMultiSystemShippingMethods($filterWebsite=false)
    {
        if (!$this->_multiSystemShippingMethods[$filterWebsite]) {
            $systemMethods = array();
            $shipping = $this->getShippingMethods($filterWebsite);
            foreach ($shipping as $s) {
                if (!$s->getSystemMethods()) {
            ...
            $this->_multiSystemShippingMethods[$filterWebsite] = $systemMethods;
        }
        return $this->_multiSystemShippingMethods[$filterWebsite];
    }

    protected $_multiSystemShippingMethodsByProfile;
    public function getMultiSystemShippingMethodsByProfile($profile, $filterWebsite=false)
    {
        $vendorCustom = false;
    ...
    public function addVendorSkus($po)
    {
        ...
        $hasVendorSku = $attr && $attr!='sku' && Mage::helper('udropship')->checkProductAttribute($attr);
        if (!empty($attrFilters)) {
            ...
            foreach ($po->getAllItems() as $item) {
                ...
                if (null === $item->getData('vendor_sku')
                    ...
                    if ($hasVendorSku && $oItem->getProductType() == 'bundle' && !$product->getSkuType() && $oItem->getChildrenItems()) {
                ...
                if (null === $item->getData('vendor_simple_sku') && !empty($simpleSkus[spl_object_hash($item)])
                    ...
                    $item->setData('__opt_sku', $optSku);
                    if ($hasVendorSku && $product->getData($attr)) {
    ...
    public function initVendorShippingMethodsForHtmlSelect($order, &$vMethods)
    {
        ...
        $oShippingDetails = Mage::helper('udropship/protected')->getOrderVendorRates($order);
    ...
    public function disableJsHtmlCompress()
    {
        Mage::app()->getStore()->setConfig('dev/js/merge_files', '0');
        if (Mage::getStoreConfig("themeframework/general/enable_compress_html")) {
            Mage::app()->getStore()->setConfig('themeframework/general/enable_compress_html', '0');
        }
    }
    ...
    static protected $_dtlIps = array('127.0.0.1','109.95.52.232','10.0.2.2');
