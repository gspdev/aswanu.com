...
class Unirgy_Dropship_Helper_Label extends Mage_Core_Helper_Abstract
{
    ...
    protected $_upsThirdPartyCfgKeys = array(
        'ups_thirdparty_account_number',
        'ups_thirdparty_country',
        'ups_thirdparty_postcode',
    );
    protected $_endiciaThirdPartyCfgKeys = array(
    );
    protected $_fedexThirdPartyCfgKeys = array(
        'fedex_payment_type',
        'fedex_thirdparty_account_number',
        'fedex_thirdparty_name',
        'fedex_thirdparty_company',
    ...
    public function beforeShipmentLabel($vendor, $track)
    {
        ...
        if (Mage::getStoreConfigFlag('udropship_label/general/use_global')) {
            if ($vendor->getUseLabelSettings()==Unirgy_Dropship_Model_Source::LABEL_SETTINGS_GLOBAL) {
                $this->useGlobalSettings($vendor, $carrierCode, $storeId);
            } elseif ($vendor->getUseLabelSettings()==Unirgy_Dropship_Model_Source::LABEL_SETTINGS_THIRDPARTY) {
    ...
    public function useGlobalSettings($vendor, $carrierCode, $storeId=null)
    {
        ...
        if ($carrierCode == 'fedex') {
            ...
            if ($vendor->getUseLabelSettings()==Unirgy_Dropship_Model_Source::LABEL_SETTINGS_THIRDPARTY) {
                $__cfgKeys = $this->_fedexThirdPartyCfgKeys;
            } else {
                $__cfgKeys = $this->_fedexCfgKeys;
            }
            foreach ($__cfgKeys as $cfgKey) {
                $vendor->setData('__'.$cfgKey, $vendor->getData($cfgKey));
            ...
            if ($vendor->getUseLabelSettings()==Unirgy_Dropship_Model_Source::LABEL_SETTINGS_THIRDPARTY) {
                $__cfgKeys = $this->_endiciaThirdPartyCfgKeys;
            } else {
                $__cfgKeys = $this->_endiciaCfgKeys;
            }
            foreach ($__cfgKeys as $cfgKey) {
                $vendor->setData('__'.$cfgKey, $vendor->getData($cfgKey));
            ...
            if ($vendor->getUseLabelSettings()==Unirgy_Dropship_Model_Source::LABEL_SETTINGS_THIRDPARTY) {
                $__cfgKeys = $this->_upsThirdPartyCfgKeys;
            } else {
                $__cfgKeys = $this->_upsCfgKeys;
            }
            foreach ($__cfgKeys as $cfgKey) {
                $vendor->setData('__'.$cfgKey, $vendor->getData($cfgKey));
