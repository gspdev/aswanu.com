...
class Unirgy_Dropship_Block_Categories extends Mage_Adminhtml_Block_Catalog_Product_Edit_Tab_Categories
{
    ...
    public function getLoadTreeUrl($expanded=null)
    {
        if ($this->hasForcedIdsString()) {
            ...
            $url = $this->getUrl($this->_udropshipCategoriesJsonUrl, array(
                '_current'=>true,
                'name_name'=>$nameName,
                'id_name'=>$idName,
            ...
            return $this->getUrl($this->_udprodCategoriesJsonUrl, array('_current'=>true));
        }
    }
    protected $_udprodCategoriesJsonUrl = 'udprod/vendor/categoriesJson';
    protected $_udropshipCategoriesJsonUrl = 'udropship/index/categoriesJson';
    ...
    protected function _getNodeJson($node, $level=1)
    {
        ...
        if (isset($item['children']) && is_array($item['children'])) {
            ...
            foreach ($children as $child) {
                if (!$this->isVendorEnabled($child['id'])) continue;
                $newChildren[] = $child;
            }
