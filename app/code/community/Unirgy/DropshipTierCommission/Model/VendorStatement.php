...
class Unirgy_DropshipTierCommission_Model_VendorStatement extends Unirgy_Dropship_Model_Vendor_Statement
{
    ...
    public function calculateOrder($order)
    {
        ...
        $order['amounts']['total_payout'] = $order['amounts']['total_payment'] = 0;
    ...
    protected function _getRefundCollection()
    {
        ...
        $refunds->getSelect()
            ...
            ->join(array('pi'=>$poType == 'po' ? $res->getTableName('udpo/po_item') : $res->getTableName('sales/shipment_item')), 'i.item_id=pi.order_item_id and t.entity_id=pi.parent_id', array('po_item_id'=>'entity_id','qty','commission_percent','base_cost','pi.order_item_id','pi.name','pi.sku','pi.vendor_sku','pi.vendor_simple_sku'))
    ...
    public function getRefundCollection($reload=false)
    {
        if (is_null($this->_refundCollection) || $reload) {
            ...
            $refundIds = array();
            foreach ($this->_refundCollection as $refund) {
                if ($refund->getRefundId()) {
                    $refundIds[$refund->getRefundId()] = 1;
                }
            }
            ...
            if ($refundIds) {
                $creditmemos = Mage::getModel('sales/order_creditmemo')->getCollection()
                    ...
                    ->addAttributeToFilter('entity_id', array('in'=>array_keys($refundIds)));
                foreach ($this->_refundCollection as $refund) {
                    $refund->setCreditmemo($creditmemos->getItemById($refund->getRefundId()));
                }
            }
        }
