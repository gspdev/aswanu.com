...
class Unirgy_DropshipVendorProduct_Model_Observer
{
    ...
    public function controller_action_layout_load_before($observer)
    {
        ...
        if ($observer->getAction()
            && in_array(strtolower($observer->getAction()->getFullActionName()), array('udprod_vendor_productedit','udprod_vendor_productnew'))
        ) {
            if (Mage::helper('udropship')->hasMageFeature('uploader')) {
                $observer->getAction()->getLayout()->getUpdate()->addHandle('_udropship_uploader_js_flow');
            } else {
                $observer->getAction()->getLayout()->getUpdate()->addHandle('_udropship_uploader_js');
            }
        }
    }
    ...
    {
        if (Mage::helper('udropship')->hasMageFeature('uploader')) {
            Mage::getConfig()->setNode('global/blocks/adminhtml/rewrite/catalog_product_helper_form_gallery_content', 'Unirgy_DropshipVendorProduct_Block_Adminhtml_CatalogProductHelperFormGalleryContentFlow');
        } else {
            Mage::getConfig()->setNode('global/blocks/adminhtml/rewrite/catalog_product_helper_form_gallery_content', 'Unirgy_DropshipVendorProduct_Block_Adminhtml_CatalogProductHelperFormGalleryContent');
        }
    ...
    public function controller_action_layout_render_before_adminhtml_udropshipadmin_vendor_edit($observer)
    {
        Mage::app()->getLayout()->getBlock('head')->setCanLoadExtJs(true);
    }
